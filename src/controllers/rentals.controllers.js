...
export async function getRentals (req, res){
    try {
        const rentalsList = await db.query(`SELECT * FROM rentals JOIN customers ON rentals."customerId" = customers.id AND JOIN games ON rentals."gameId" = games.id`)
...
export async function postNewRental (req, res){
    ...
    try {
        ...
        const originalPrice = Number(daysRented) * Number(isGame.rows[0].pricePerDay);
